...
namespace CSharp
{
    class Parallelism
    {
        ...
        /// 2. A task represents an operation that is running or going to run
        /// 3. In some ways, a task resembles a thread or ThreadPool work item, but at a higher level of abstraction. 
        /// 4. The term task parallelism refers to one or more independent tasks running concurrently. 
        ///	5. Tasks provide two primary benefits:
        ...
        /// 6. For both of these reasons, in the.NET Framework, TPL is the preferred API for writing multi-threaded, asynchronous, 
        ///    and parallel code.
        /// </summary>
        public void TaskBasics()
        {
            ...
            var task1 = new Task<double>(() => { return ToSquare(10, 2000); });
            task1.Start();
            Console.WriteLine($"Value returned form task1 from >>> {task1.Result}");

            var task2 = new Task<double>(() => { return ToSquare(20, 5000); });
            task2.Start();
            Console.WriteLine($"Value returned form task2 from >>> {task2.Result}");

            var task3 = new Task<double>(() => { return ToSquare(30, 3000); });
            task3.Start();
            Console.WriteLine($"Value returned form task3 from >>> {task3.Result}");
        ...
        public double ToSquare(int value, int delay)
        {
            Thread.Sleep(delay);
            var square = Math.Pow(value, 2);
            Console.WriteLine($"Square of {value} is {square} from Thread {Thread.CurrentThread.ManagedThreadId}");
            return square;
        }
